...
def fake_button():
    '''
    def - функция, бесконечно вызывающая себя при нажании на кнопку "Переход на пятый уровень"
    window.close() - каждый закрывает предыдущее окно
    k - наполнение окна
    location=(random.randint(0, 1500), random.randint(0, 1000)) - рандомное расположение окна (в пределах экрана), что делает из окна - "бегающую кнопку"
    '''
    window.close()
...
def picture():
    '''
    window10.close() - закрытие предыдущего окна
    image - элемент, помогающий открыть картинку
    '''
    window10.close()
...
def mistake():
    '''
    def - вызывается при вводе неверного ответа в текстовое поле
    k - наполнение окна
    '''
...
def start1():
    '''
    def - вызывается сразу при запуске проекта (стартовое окно)
    k - наполнение окна
    location - расположение окна на экране
    '''
...
def level1():
    '''
    def - вызывается при переходе на первый уровень
    k - наполнение окна
    '''
    window.close()
...
def level2():
    '''
    def - вызывается при переходе на второй уровень
    k - наполнение окна
    '''
    window2.close()
...
def level3():
    '''
    def - вызывается при переходе на третий  уровень
    k - наполнение окна
    self_image - элемент, записывается путь и после открывается картинка
    '''
    window4.close()
...
def level4():
    '''
    def - вызывается при переходе на четвертый уровень
    k - наполнение окна
    '''
    window6.close()
    k = [[psg.Text('Нажмите на кнопку - Переход на пятый уровень')],
...
def level5():
    '''
    window, event, values - переменные в которые считываются данные об окне
    window.close() - закрывает предыдущее окно
    def - вызывается при переходе на пятый уровень
    k - наполнение окна
    psg.text - строки с заданием
    '''
...
def level6():
    '''
    window.close() - закрывает предыдущее окно
    def - вызывается при переходе на шестой уровень
    k - наполнение окна
    '''
    window8.close()
...
def level7():
    '''
    window.close() - закрывает предыдущее окно
    def - вызывается при переходе на седьмой уровень
    k - наполнение окна
    '''
    window9.close()
...
def information():
    '''
    def - вызывается при нажатии на кнопку "Руководство" уровень
    k - наполнение окна
    psg.text - строки с справочной информацией ("правилами")
    '''
